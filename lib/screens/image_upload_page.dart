...
class _ImageUploadPageState extends State<ImageUploadPage> {
    ...
    Future<void> _pickImages() async {
        ...
        if (!granted) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('갤러리 권한이 필요합니다')));
            return;
        }
    ...
    Future<void> _upload() async {
        if (!_consented) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('약관 동의가 필요합니다')));
            return;
        }
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Row(children: [
                                ElevatedButton(
                                    ...
                                    style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF00C2FD)),
                                    child: Text('이미지 선택', style: GoogleFonts.inter(color: Colors.white, fontWeight: FontWeight.w700)),
                                ),
                                const SizedBox(width: 12),
                                OutlinedButton(onPressed: _loading ? null : _upload, child: const Text('업로드')),
                            ]),
                            const SizedBox(height: 12),
                            Row(children: [
                                Checkbox(value: _consented, onChanged: (v) => setState(() { _consented = v ?? false; })),
                                Expanded(child: Text('이미지 업로드 약관에 동의합니다', style: GoogleFonts.inter(fontSize: 13, color: const Color(0xFF777777))))
                            ]),
                            if (_loading) ...[
                                const SizedBox(height: 12),
                        ...
                        child: GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                                ...
                                return Stack(children: [
                                    ...
                                    Positioned(top: 4, right: 4, child: InkWell(onTap: () { setState(() { _selected.removeAt(index); }); }, child: Container(decoration: const BoxDecoration(color: Colors.black54, shape: BoxShape.circle), padding: const EdgeInsets.all(4), child: const Icon(Icons.close, size: 16, color: Colors.white))))
                                ]);
                            },
                        ),
                    )
